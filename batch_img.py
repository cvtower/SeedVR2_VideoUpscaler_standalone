...
def extract_frames_from_video(video_path, debug=False, skip_first_frames=0, load_cap=None):
    ...
    image_extensions = ('.jpg', '.jpeg', '.JPG', '.png', '.PNG', '.bmp', '.gif', '.tiff', '.webp')
    filelist = os.listdir(video_path)
    frames = []
    item_names = []
    for item in filelist:
        if item.endswith((image_extensions)):
            vid_item = video_path+item
            if not os.path.exists(vid_item):
                raise FileNotFoundError(f"Video file not found: {vid_item}")
            
            # Open video
            cap = cv2.VideoCapture(vid_item)
            if not cap.isOpened():
                raise ValueError(f"Cannot open video file: {vid_item}")
    ...
    return frames_tensor, fps, item_names
...
def save_frames_to_png(frames_tensor, output_dir, item_names, debug=False):
    """
...
def parse_arguments():
    ...
    parser.add_argument("--video_path", type=str, default='img_src/img_src/b1/',
                        help="Path to input video file")
    parser.add_argument("--seed", type=int, default=100,
                        help="Random seed for generation (default: 100)")
    parser.add_argument("--resolution", type=int, default=1200,#1456,
    ...
    parser.add_argument("--output_format", type=str, default="png", choices=["video", "png"],
...
def main():
    ...
    try:
        ...
        frames_tensor, original_fps, item_names = extract_frames_from_video(
            args.video_path, 
            args.debug, 
        ...
        if args.output_format == "png":
            ...
            #base_name = Path(args.video_path+item).stem #+ "_upscaled"
            if args.debug:
                print(f"🖼️ Saving PNG frames to directory: {output_dir}")
            save_start = time.time()
            save_frames_to_png(result, output_dir, item_names, args.debug)
            if args.debug:
